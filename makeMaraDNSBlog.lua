...
gOutHandle = io.open("entries/MaraDNS.html","wb")
if not gOutHandle then
  print("Fatal error opening entries/MaraDNS.html for writing")
  os.exit(1)
end
...
fo('<html><head><title>MaraDNS Blog</title>')
fo(U) 
fo('<style>')
...
fo('<div class=blog>')
fo('<a href="https://samboy.github.io/MaraDNS/webpage/">MaraDNS webpage</a>')
fo(' - ')
fo('<a href="#GitBlogIndex">MaraDNS Blog Index</a></i>')
fo('</div><div class=blog>')
fo('<h1>MaraDNS blog</h1>')
fo('')
...
-- Go through files in reverse sorted order (newest blogs first)
...
doContinue = false
  for fileName in sPairs(fileList,function(a,b) return b<a end) do
    ...
    local title=nil
    local inTitle = false
    -- Grab title; see if it’s a MaraDNS entry
    for line in thisFileHandle:lines() do
      ...
      if line:match('%<%!%-%-%{TAGS%:M') then isMaraDNS = true end
      if line:match('pics%/MaraDNS%.jpg') then isMaraDNS = true end
    end
    thisFileHandle:close()
    if not title then title="MaraDNS entry" end

    fo('<!-- `ENTRY: ' .. date .. ' ' .. year .. ' ` -->')
    fo('<a name="' .. link .. '"> </a>')

    if isMaraDNS then
      blogIndex = blogIndex .. '<a href="#' .. link .. '">' 
                  .. title .. ' (' .. date .. ')</a></br>' .. "\n"
      thisFileHandle = io.open("embed/" .. fileName,"rb")
      if not thisFileHandle then
        print("Fatal error: Second open of embed/" .. fileName .. " failed")
        os.exit(3)
      end
      ...
      for l in thisFileHandle:lines() do
        ...
        elseif relativeLink and not isIndex and 
               l:match('href=[^"]') then
          l = l:gsub('href=([^%"%#%/])','href=../%1')
        end
...
fo('</div>')
fo('</body></html>')
print('entries/MaraDNS.html made')
