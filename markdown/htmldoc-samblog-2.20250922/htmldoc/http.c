  ...
  while (lineptr < lineend)
  {
    ...
    */
	...
	if (http->timeout_cb && (*http->timeout_cb)(http, http->timeout_data))
      ...
      DEBUG_printf(("4httpGets: read " HTMLDOC_LLFMT " bytes.", HTMLDOC_LLCAST bytes));
	 ...
	 size_t length)			/* I - Maximum number of bytes */
{
  ...
  DEBUG_printf(("httpPeek(http=%p, buffer=%p, length=" HTMLDOC_LLFMT ")", (void *)http, (void *)buffer, HTMLDOC_LLCAST length));
  ...
  DEBUG_printf(("2httpPeek: data_remaining=" HTMLDOC_LLFMT,
...
#endif /* HAVE_LIBZ */
  {
    ...
    */
    ...
    DEBUG_printf(("2httpPeek: Read " HTMLDOC_LLFMT " bytes into buffer.",
	   ...
	   ...)				/* I - Additional args as needed */
{
  ...
  DEBUG_printf(("3httpPrintf: (" HTMLDOC_LLFMT " bytes) %s", HTMLDOC_LLCAST bytes, buf));
...
#ifdef HAVE_LIBZ
  DEBUG_printf(("httpRead2(http=%p, buffer=%p, length=" HTMLDOC_LLFMT ") coding=%d data_encoding=%d data_remaining=" HTMLDOC_LLFMT, (void *)http, (void *)buffer, HTMLDOC_LLCAST length, http->coding, http->data_encoding, HTMLDOC_LLCAST http->data_remaining));
#else
  DEBUG_printf(("httpRead2(http=%p, buffer=%p, length=" HTMLDOC_LLFMT ") data_encoding=%d data_remaining=" HTMLDOC_LLFMT, (void *)http, (void *)buffer, HTMLDOC_LLCAST length, http->data_encoding, HTMLDOC_LLCAST http->data_remaining));
#endif /* HAVE_LIBZ */
	...
	DEBUG_printf(("2httpRead2: avail_in=%d, avail_out=%d, bytes=%d",
	  ...
	  ((z_stream *)http->stream)->next_in = http->sbuffer;
	    ...
	    bytes = http_read_buffered(http, (char *)http->sbuffer + ((z_stream *)http->stream)->avail_in, (size_t)buflen);
          ...
          DEBUG_printf(("1httpRead2: Adding " HTMLDOC_LLFMT " bytes to "
                        "decompression buffer.", HTMLDOC_LLCAST bytes));
		...
		size_t urilen)		/* I - Size of URI buffer */
{
  ...
  */
  ...
  DEBUG_printf(("httpReadRequest(http=%p, uri=%p, urilen=" HTMLDOC_LLFMT ")", (void *)http, (void *)uri, HTMLDOC_LLCAST urilen));
...
{
  DEBUG_printf(("httpSetLength(http=%p, length=" HTMLDOC_LLFMT ")", (void *)http, HTMLDOC_LLCAST length));
  ...
  else
  {
    ...
    snprintf(len, sizeof(len), HTMLDOC_LLFMT, HTMLDOC_LLCAST length);
	   ...
	   size_t     length)		/* I - Number of bytes to write */
{
  ...
  DEBUG_printf(("httpWrite2(http=%p, buffer=%p, length=" HTMLDOC_LLFMT ")", (void *)http, (void *)buffer, HTMLDOC_LLCAST length));
  ...
  */
  ...
  DEBUG_printf(("1httpWrite2: Returning " HTMLDOC_LLFMT ".", HTMLDOC_LLCAST bytes));
...
//    httpSetField(http, HTTP_FIELD_SERVER, http->default_fields[HTTP_FIELD_SERVER] ? http->default_fields[HTTP_FIELD_SERVER] : CUPS_MINIMAL);
    httpSetField(http, HTTP_FIELD_SERVER, http->default_fields[HTTP_FIELD_SERVER] ? http->default_fields[HTTP_FIELD_SERVER] : "HTMLDOC/" SVERSION);
          ...
          size_t length)		/* I - Maximum bytes to read */
{
  ...
  DEBUG_printf(("http_read(http=%p, buffer=%p, length=" HTMLDOC_LLFMT ")", (void *)http, (void *)buffer, HTMLDOC_LLCAST length));
                   ...
                   size_t length)	/* I - Maximum bytes to read */
{
  ...
  DEBUG_printf(("http_read_buffered(http=%p, buffer=%p, length=" HTMLDOC_LLFMT ") used=%d", (void *)http, (void *)buffer, HTMLDOC_LLCAST length, http->used));
...
http_read_chunk(http_t *http,		/* I - HTTP connection */
		...
		size_t length)		/* I - Maximum bytes to read */
{
  DEBUG_printf(("http_read_chunk(http=%p, buffer=%p, length=" HTMLDOC_LLFMT ")", (void *)http, (void *)buffer, HTMLDOC_LLCAST length));
  ...
  if (http->data_remaining <= 0)
  {
    ...
    if (http->data_remaining < 0)
    {
      DEBUG_printf(("1http_read_chunk: Negative chunk length \"%s\" ("
                    HTMLDOC_LLFMT ")", len, HTMLDOC_LLCAST http->data_remaining));
      return (0);
    }

    DEBUG_printf(("2http_read_chunk: Got chunk length \"%s\" (" HTMLDOC_LLFMT ")",
                  len, HTMLDOC_LLCAST http->data_remaining));
  ...
  DEBUG_printf(("2http_read_chunk: data_remaining=" HTMLDOC_LLFMT,
                HTMLDOC_LLCAST http->data_remaining));
...
http_set_length(http_t *http)		/* I - Connection */
{
  ...
  if ((remaining = httpGetLength2(http)) >= 0)
  {
    ...
    DEBUG_printf(("1http_set_length: Setting data_remaining to " HTMLDOC_LLFMT ".",
                  HTMLDOC_LLCAST remaining));
	   ...
	   size_t     length)		/* I - Number of bytes to write */
{
  ...
  DEBUG_printf(("2http_write(http=%p, buffer=%p, length=" HTMLDOC_LLFMT ")", (void *)http, (void *)buffer, HTMLDOC_LLCAST length));
  http->error = 0;
  tbytes      = 0;
...
#  endif /* _WIN32 */
    ...
    DEBUG_printf(("3http_write: Write of " HTMLDOC_LLFMT " bytes returned "
                  HTMLDOC_LLFMT ".", HTMLDOC_LLCAST length, HTMLDOC_LLCAST bytes));
  ...
  DEBUG_printf(("3http_write: Returning " HTMLDOC_LLFMT ".", HTMLDOC_LLCAST tbytes));
		 ...
		 size_t        length)	/* I - Length of buffer */
{
  ...
  DEBUG_printf(("7http_write_chunk(http=%p, buffer=%p, length=" HTMLDOC_LLFMT ")", (void *)http, (void *)buffer, HTMLDOC_LLCAST length));
