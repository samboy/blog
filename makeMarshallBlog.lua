...
gOutHandle = io.open("entries/Marshall.html","wb")
if not gOutHandle then
  print("Fatal error opening entries/Marshall.html for writing")
  os.exit(1)
end
...
fo('<html><head><title>Memories of Marshall</title>')
fo(U) 
fo('<style>')
...
fo('<div class=blog>')
fo('<a href="https://samboy.github.io/blog/">Current blog entries</a>')
fo(' - ')
fo('<a href="../archive.html#GitBlogIndex">Full blog Index</a></i>')
fo('</div><div class=blog>')
fo('<h1>Memories of Marshall</h1>')
fo('')
...
doContinue = false
  for fileName in sPairs(fileList,function(a,b) return b<a end) do
    ...
    local title=nil
    local inTitle = false
    -- Grab title; see if it’s a Marshall entry
    for line in thisFileHandle:lines() do
      ...
      if line:match('pics%/[Mm]arshall') then isMarshall = true end
    end
    thisFileHandle:close()
    if not title then title="Marshall Blog" end
...
fo('</div>')
fo('</body></html>')
print('entries/Marshall.html made')
